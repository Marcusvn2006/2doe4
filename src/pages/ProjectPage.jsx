...
function ShareBar({ title, subtitle }) {
  ...
  const shareNative = async () => {
    if (navigator.share) {
      ...
      setCopied(true)
      toast.success('Link copiado!', { description: 'Cole e compartilhe onde quiser.' })
  ...
  return (
    <div className="flex items-center gap-2 mt-5">
      ...
      <button
        onClick={shareNative}
        title={navigator.share ? 'Compartilhar' : 'Copiar link'}
      ...
      >
        <div className="max-w-[1600px] mx-auto px-6 sm:px-10 lg:px-16 xl:px-24 w-full pt-20 sm:pt-32 pb-16">
          ...
          <div className="max-w-[320px] sm:max-w-sm lg:max-w-md">
            ...
            <FadeUp delay={0.36}>
              <ShareBar title={title} subtitle={subtitle} />
            </FadeUp>
          </div>
        </div>
      </section>
